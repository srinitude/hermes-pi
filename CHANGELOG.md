...
- Adds companion-repo scaffolding: `install.sh`, `mise.toml`, `pyproject.toml`,
  `README.md`, `LICENSE`, `.gitignore`, `CHANGELOG.md`, `CONTRIBUTING.md`,
  `SECURITY.md`, `.github/workflows/ci.yml`.
- Centralizes every contributor task under `mise.toml` (sole project task
  registry); `mise run <task>` is the only supported entrypoint and there is
  no `Makefile` or Make compatibility shim.
- Adds repo-sync invariant tests under `tests/test_repo_*` /
  ...
  `tests/test_mise_tasks.py` / `tests/test_readme_mise_only.py`.
- `mise run link` replaces the in-Hermes skill directory with a symlink to
  this checkout, after backing the original up to
